...
- **加密**: AES-256-GCM (pointycastle) + PBKDF2-HMAC-SHA256 密钥派生（100K 迭代）
...
- 临时解密文件操作完成后立即删除
- PIN 输入框禁止粘贴
- FLAG_SECURE 防截屏按页面控制（ScreenSecureMixin），仅敏感页面启用（folder_detail、image_preview、video_player、trash）
- PBKDF2 迭代次数固定 100K（600K 在移动端需 11.5 秒，不可用）
- CPU 密集操作（PBKDF2、加密）必须在 Isolate.run 中执行
...
- `flutter test` — 运行单元测试
- `flutter test test/integration/` — 运行集成测试
- `dart run build_runner build` — 重新生成 drift 代码

## 关键文件

- `lib/main.dart` — 应用入口，DI 初始化
- `lib/app.dart` — GoRouter 路由表、全局 BLoC Provider
- `lib/core/crypto/key_manager.dart` — KEK/DEK 密钥管理核心
- `lib/core/storage/encrypted_file_storage.dart` — PVLT 文件读写
- `android/.../KeystorePlugin.kt` — Android Keystore 原生桥接

## 仓库

- GitHub: h3c-hexin/privacy-vault (private)
